...
# Fungsi untuk menukar imej tempatan ke format Base64 (untuk diletakkan dalam HTML)
...
def login_page():
    ...
    with col2:
        ...
        # Gunakan logo PUO tempatan di halaman login juga
        # Sila pastikan fail logo PUO disimpan sebagai 'puo_logo.png' dalam folder yang sama
        try:
            st.image("puo_logo.png", width=150)
        except:
            # Jika fail imej tiada, gunakan imej lalai dari URL
...
def main_app():
    ...
    # --- TAJUK UTAMA DENGAN LOGO PUO (KEMASKINI BARU) ---
    # Sila pastikan fail logo PUO disimpan sebagai 'puo_logo.png' dalam folder yang sama
    logo_path = "puo_logo.png"
    try:
        # Menukar fail tempatan 'puo_logo.png' kepada format base64 untuk diletakkan dalam HTML
        with open(logo_path, "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        
        st.markdown(f"""
            <div style='display: flex; align-items: center; gap: 20px; border-left: 5px solid #007bff; padding: 10px 20px;'>
                <img src='data:image/png;base64,{encoded_string}' width='100' style='flex-shrink: 0;'>
                <div>
                    <h1 style='margin: 0; font-size: 32px;'>SISTEM SURVEY LOT PUO</h1>
                    <p style='color: #6c757d; margin: 0; font-size: 16px;'>Politeknik Ungku Omar | Jabatan Kejuruteraan Awam</p>
                </div>
            </div>
            <br>
        """, unsafe_allow_html=True)
        ...
        # Jika fail 'puo_logo.png' tiada, gunakan imej dari URL sebagai 'backup'
        st.markdown(f"""
            <div style='display: flex; align-items: center; gap: 20px; border-left: 5px solid #007bff; padding: 10px 20px;'>
                <img src='https://www.puo.edu.my/wp-content/uploads/2021/08/cropped-LOGO-PUO-1.png' width='80' style='flex-shrink: 0;'>
                <div>
                    <h1 style='margin: 0; font-size: 32px;'>SISTEM SURVEY LOT PUO</h1>
                    <p style='color: #6c757d; margin: 0; font-size: 16px;'>Politeknik Ungku Omar | Jabatan Kejuruteraan Awam</p>
                </div>
            ...
            <br>
        """, unsafe_allow_html=True)
        st.warning("Peringatan: Fail imej tempatan 'puo_logo.png' tidak dijumpai. Sistem menggunakan logo lalai.")
