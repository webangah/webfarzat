            ...
            with tab_map:
                ...
                if lats is not None:
                    ...
                    for i, row in df.iterrows():
                        ...
                        folium.Marker(
                            [lats[i], lons[i]], 
                            icon=folium.DivIcon(html=f'<div style="font-size: 10pt; color: white; font-weight: bold; background: rgba(0,0,0,0.5); padding: 2px;">{stn_label}</div>')
                        ).add_to(m)

                    # Fit bounds supaya automatik zoom ke poligon
                    m.fit_bounds(points)
