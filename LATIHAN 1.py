...
def transform_coords(df, epsg_code):
    try:
        # EPSG:4390 (Timbalai 1948 / RSO Borneo) ke WGS84
...
def main_app():
    ...
    # Guna data point.csv yang dihantar jika fail dimuat naik
    if uploaded_file is not None:
        ...
        if 'E' in df.columns and 'N' in df.columns:
            ...
            # --- TAB PAPARAN ---
            tab_map, tab_plot = st.tabs(["🌍 Peta Satelit (Google)", "📊 Lukisan Teknikal"])
            ...
            with tab_map:
                ...
                if lats is not None:
                    # Setup Folium Map - Purata koordinat sebagai pusat permulaan
                    ...
                    folium.Polygon(locations=points, color="white", weight=3, fill=True, fill_color=warna_poli, fill_opacity=0.5).add_to(m)
                    ...
                    for i, row in df.iterrows():
                        ...
                        folium.Marker(
                            [lats[i], lons[i]], 
                            icon=folium.DivIcon(html=f'<div style="font-size: 10pt; color: white; font-weight: bold; background: rgba(0,0,0,0.5); padding: 2px; border-radius: 3px;">{stn_id}</div>')
                        ).add_to(m)

                    # --- FIX: Auto Zoom ke Poligon ---
                    m.fit_bounds(points)
                    
                    st_folium(m, width=1100, height=500, returned_objects=[])
                else:
                    st.error("Gagal menukar koordinat. Sila pastikan Kod EPSG (4390) betul untuk data anda.")
            ...
            with tab_plot:
                ...
                # Label bearing/jarak pada plot matplotlib
                for i in range(len(df)):
                    x1, y1 = e[i], n[i]
                    next_idx = (i + 1) % len(df)
                    x2, y2 = e[next_idx], n[next_idx]
                    dist = np.sqrt((x2-x1)**2 + (y2-y1)**2)
                    brg = np.degrees(np.arctan2((x2-x1), (y2-y1))) % 360
                    ax.text((x1+x2)/2, (y1+y2)/2, f"{to_dms(brg)}\n{dist:.2f}m", fontsize=saiz_teks, color='red', ha='center')
